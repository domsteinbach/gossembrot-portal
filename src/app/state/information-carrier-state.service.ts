...
export class UpdateCarriers {
  ...
  constructor(public carriers: InformationCarrier[]) {}
}
...
export class CarriersState {
  ...
  @Selector()
  static existingManuscripts(
    state: InformationCarrier[],
  ): InformationCarrier[] {
    return state
      .filter(
        (carrier) => !carrier.isLost && carrier.carrierType === "Manuscript",
      )
  ...
  updateInformationCarrier(
    ...
    if (
      ...
      informationCarrier.id ===
        this._store.selectSnapshot(SelectedSrcInformationCarrierState)
    ) {
      return;
    }
    combineLatest([
      ...
      .pipe(take(1))
      .subscribe(([texts = [], pages = []]) => {
        this._store.dispatch(
          new UpdateSelectedCarriersTexts(
            texts.sort((a, b) => a.sortInCar - b.sortInCar),
          ),
        );
        this._store.dispatch(new UpdateSelectedSrcCarrierPages(pages));

        const textToSelect = carrierTextId
          ? texts.find((t) => t.id === carrierTextId)
          : null;

        if (textToSelect) {
          this._store.dispatch(new UpdateSelectedCarrierText(textToSelect));

          const pagesOfText = pages
            .filter((p) => p.textId === textToSelect.id)

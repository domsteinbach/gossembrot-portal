...
@Component({
  ...
  standalone: true,
  imports: [MatFormField, MatInput, MatLabel, NgForOf, NgIf],
...
export class TableFilterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.displayedFilters$ = this._ds
      .getFiltersToDisplay$(this.tableName)
      .pipe(map((columns) => columns.filter((c) => c.displayFilter)));
  ...
  onDisplayFilterChange(newColumnDefs: ColumnDef[]): void {
    ...
    const removedColumns = this.displayedColumnFilters.filter(
      (c) => !newColumnDefs.includes(c),
    );
    const addedColumns = newColumnDefs.filter(
      (c) => !this.displayedColumnFilters.includes(c),
    );

    if (removedColumns.length > 0) {
      const filters: string = this._fs.updateStringContentFilters(
        this.tableName,
        removedColumns[0].column,
        "",
      );

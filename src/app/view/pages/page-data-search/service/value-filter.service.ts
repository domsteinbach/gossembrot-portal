...
export class ValueFilterService {
  private _activeFiltersPerTable: Map<TableName, Map<Column, string>> = new Map<
    TableName,
    Map<Column, string>
  >();

  private _activeNullFiltersPerTable: Map<TableName, NullFilter[]> = new Map<
    TableName,
    NullFilter[]
  >();

  private _activeNullFiltersSubject = new BehaviorSubject<
    Map<TableName, NullFilter[]>
  >(new Map<TableName, NullFilter[]>());
  activeNullFilters$ = this._activeNullFiltersSubject.asObservable();

  getMultiFilterPredicate<T extends GsmbResource>(
    keysToSkip: (keyof T)[] = [],
  ) {
    return (data: T, filter: string): boolean => {
      ...
      return (Object.keys(filters) as (keyof T)[]).every((key) => {
        ...
        return filterValue
          ? dataValue?.includes(filterValue.toLowerCase())
          : true;
      });
    };
  }

  updateStringContentFilters(
    tableName: TableName,
    column: Column,
    inputValue: string,
  ): string {
    const filters =
      this._activeFiltersPerTable.get(tableName) || new Map<Column, string>();
  ...
  updateNullFilter(tableName: TableName, nullFilter: NullFilter) {
    ...
    const index = nullFilters.findIndex(
      (nf) => nf.column.column === nullFilter.column.column,
    );
    if (index !== -1) {
  ...
  applyNullFilters<T>(data: T[], tableName: TableName): T[] {
    const nullFilters = this._activeNullFiltersPerTable.get(tableName);
    if (!nullFilters) {
      return data;
    }
    return data.filter((entry) =>
      this._filterByNullFilters(entry, nullFilters),
    );
  }

  private _filterByNullFilters<T>(
    entry: T,

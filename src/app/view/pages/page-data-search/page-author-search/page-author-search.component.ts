...
export class AuthorSearchComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private _authors$ = combineLatest([
    this._ar.authors$(),
    this._fs.nullFilters$("author"),
  ]).pipe(
    map(([authors, filters]) =>
      this._fs.applyNullFilters<Author>(authors, "author"),
    ),
  );
    ...
    this._authors$
      ...
      .subscribe((authors: Author[]) => {
        this.dataSource.data = authors.sort((a, b) => {
          if (
            a.cognomen.replace("(Ps.-)", "") < b.cognomen.replace("(Ps.-)", "")
          ) {
            return -1;
          }
          if (
  ...
  ngAfterViewInit() {
    ...
    this.dataSource.filterPredicate =
      this._fs.getMultiFilterPredicate<Author>();
    ...
    this.dataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case "gndData.dateOfBirth":
          return (
            (item as Author).gndData?.dateOfBirth
              ?.replace("'", "")
              .replace("[", "")
              .toLowerCase() || ""
          );

...
export class VerweisSynopsisComponent implements OnInit, OnDestroy {
  ...
  targetCarriers$ = this._store.select(CarriersState).pipe(
    filter((carriers) => carriers.length > 0),
    map((filter) =>
      filter.filter(
        (carrier: InformationCarrier) => carrier.hasIncomingVerweis,
      ),
    ),
  );
  ...
  ngOnInit() {
    const verweisToInit = this._route.snapshot.queryParamMap.get(
    ...
    if (verweisToInit) {
      combineLatest([
        this.srcCarriers$,
        this._vr.getVerweisById$(verweisToInit),
      ])
        .pipe(
      ...
      this.srcCarriers$.subscribe((carriers) => {

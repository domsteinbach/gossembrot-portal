...
export class InGsmbComponent {
  ...
  manuscriptsInGsmb$ = this._store.select(CarriersState)
    .pipe(
      map(carriers => carriers.filter((carrier: InformationCarrier) => carrier.isLost && carrier.inGsmbsLib && carrier.carrierType === 'Manuscript')));

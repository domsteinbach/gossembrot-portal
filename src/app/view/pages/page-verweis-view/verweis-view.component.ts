...
export class VerweisViewComponent implements OnInit {
  ...
  get commentText(): string {
    const priority =
      this.data.verweis.insecurity > 0
        ? `Priorität ${this.data.verweis.insecurity}. `
        : "";
  ...
  getTargetPage(): Page | undefined {
    return this.data.verweis.targetCarObj?.physicality !== "Available"
      ? new PageOfMissingCarrier()
      : this.data.verweis.targetBelegstelleObj?.getPageOrAlternativePage();
  }

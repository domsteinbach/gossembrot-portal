...
import {
  ...
  VerweisViewData,
} from "../page-verweis-view/verweis-view.component";
...
export class LinkService {
  ...
  openTextInNewTab(text: CarrierText): void {
    ...
    window.open(
      `${baseRoute}?${RouteConstants.QUERY_CARRIERTEXT_PARAM}=${text.id}`,
      "_blank",
    );
  }
  ...
  private _getExistingTargetRoute(verweis: DisplayVerweis): string {
    const page = verweis.targetBelegstelleObj?.pageId
      ? verweis.targetBelegstelleObj.pageId
      : verweis.targetBelegstelleObj?.alternativePageId || "";
  ...
  openVerweisViewDialog(
    verweis: DisplayVerweis,
    view: View,
    hideSynopsisButton = false,
  ): void {
    ...
    switch (carrier.physicality) {
      ...
      case "Lost":
        if (carrier.inGsmbsLib && carrier.carrierType === "Manuscript") {
          route =
            route +
            `${RouteConstants.RECONSTRUCTION}/${RouteConstants.IN_GSM_VALUE}/${carrier.id}`;
        } else if (
          !carrier.inGsmbsLib &&
          ...
          route =
            route +
            `${RouteConstants.RECONSTRUCTION}/${RouteConstants.OUT_GSM_VALUE}/${carrier.id}`;
        } else {
          route =
            route +
            `${RouteConstants.RECONSTRUCTION}/${RouteConstants.PRINTS_VALUE}/${carrier.id}`;
        }
        break;
      case "Classic":

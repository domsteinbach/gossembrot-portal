...
import {
  CarriersState,
  SelectedSrcInformationCarrierState, UpdateSelectedSrcInformationCarrier,
...
export class ManuscriptComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.selectedVerweis$.pipe(
      filter(Boolean),
      takeUntil(this._destroy$)).subscribe(v => {
      this._store.dispatch(new UpdateSelectedPage(v?.srcBelegstelleObj?.page));
    });

    this._initializeSourceCarrierFromParams(this._route.firstChild?.snapshot.paramMap, this._route.snapshot.queryParamMap);
  }

  private _initializeSourceCarrierFromParams(paramMap?: ParamMap, queryParamMap?: ParamMap): void {
    this._store.select(CarriersState.existingManuscripts).pipe(
  ...
  private _initCarrierFromParams(carriers: InformationCarrier[], paramMap?: ParamMap, queryParamMap?: ParamMap): void {
    const carParam = paramMap?.get(RouteConstants.INFO_CARRIER_PARAM);
    const pageParam = queryParamMap?.get(RouteConstants.QUERY_PAGE_PARAM) || undefined;
    const carrierTextParam = queryParamMap?.get(RouteConstants.QUERY_CARRIERTEXT_PARAM) || undefined;
    const selectedCarrier = carriers.find((c) => c.id === carParam) || carriers[0];
    if (!carParam) { // set the param
      this._router.navigate([RouteConstants.MANUSCRIPTS, selectedCarrier.id], {
        queryParamsHandling: 'merge',
      });
    }
    this._store.dispatch(new UpdateSelectedSrcInformationCarrier(selectedCarrier, pageParam, carrierTextParam));
  }

...
export class SearchResultGroupComponent implements OnChanges {
  ...
  get carrierTexts(): CarrierText[] {
    return this._carrierTexts.slice(
      0,
      this.showAll ? undefined : this.search.OFFSET,
    );
  }
  ...
  get authorsWithTexts() {
    return this._authorsWithTexts.slice(
      0,
      this.showAll ? undefined : this.search.OFFSET,
    );
  }
  ...
  get namingsWithCarriers() {
    return this._namingsWithCarriers.slice(
      0,
      this.showAll ? undefined : this.search.OFFSET,
    );
  }
  ...
  private _getAuthorsWithTexts(): void {
    const authorIds = this.results
      .filter((r) => r.type === "author" && this._included(r))
      .map((r) => r.id);
    this._ctr
      .getCarrierTextsOfAuthors$(authorIds)
      .pipe(take(1))
      .subscribe((texts) => {
  ...
  private _getCarrierTexts(): void {
    const carrierTextIds = this.results
      .filter((r) => r.type === "carrier_text")
      .map((r) => r.id);
    this._ctr
      .getCarrierTextsByIds$(carrierTextIds)
      .pipe(take(1))
      .subscribe((texts) => {
  ...
  _getNamingsWithCarrier() {
    this._icr
      ...
      .pipe(take(1))
      .subscribe((carriers) => {
        this._namingsWithCarriers = this.results.map((result) => {
          const c = carriers.find((c) =>
            c.namingsGossembrot.some((n) => n.id === result.id),
          );
          return {
            naming: result,
            carrier: c,
  ...
  private _included(result: SearchResult) {
    return (
      this.includeAuthorsFoundByAlias || result.label.includes(this.searchTerm)
    );
  }
  ...
  private _getFoundByAlias(result: SearchResult): string | undefined {
    ...
    const aliases = result.searchString.split("|").map((alias) => alias.trim());
  ...
  onResultItemClicked(result: SearchResult): void {
    switch (result.type) {
      ...
      case "naming_gossembrot":
        this._icr
          .getCarrierByNamingGsmbId$(result.id)
          .pipe(take(1))
          .subscribe((carrier) => {
            if (carrier) {
              this._ls.openCarrierInNewTab(carrier);

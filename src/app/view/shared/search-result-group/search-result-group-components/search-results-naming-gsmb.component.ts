  ...
  template: `
    <div
      ...
      *ngFor="let result of results"
      (click)="onTextClicked(result.carrier)"
      tabindex="0"
      (keydown.enter)="onTextClicked(result.carrier)"
      (keydown.space)="onTextClicked(result.carrier)"
    >
      <div class="value-item">
        ...
        <span>
          [{{
            result.carrier?.getCarrierTypeDescDeLowerCased("Nominativ")
          }}</span
        >
        <span
          [class.lost-carrier-title]="result.carrier?.physicality === 'Lost'"
        >
          {{ result.carrier?.fullTitle }}]</span
        >
      </div>
    </div>
  `,

...
@Component({
  ...
  template: `
    <div class="suggestion-item" *ngFor="let text of textsToDisplay;" (click)="onTextClicked(text)">
      <div class="value-item">📖 <span
        [innerHTML]="text.title | highlight:searchTerm"></span><span> [in {{ text.carrier?.getCarrierTypeDescDeLowerCased('Dativ') }}</span>
        <span [class.lost-carrier-title]="text.carrier?.physicality === 'Lost'"> {{ text.carrierFulltitle }}]</span></div>
    </div>
...
export class SearchResultCarriertextComponent {
  ...
  get textsToDisplay() {
    return this.texts.slice(0, this._showAll || !this.showAllHandler? undefined : this.search.OFFSET);
  }

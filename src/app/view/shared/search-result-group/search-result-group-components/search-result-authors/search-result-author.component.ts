...
@Component({
  ...
  styleUrls: [
    "./search-result-author.component.scss",
    "../../search-result-group.component.scss",
  ],
...
export class SearchResultAuthorComponent {
  ...
  toggleTexts(id: string): void {
    if (
      !this.authorsWithTexts.length ||
      !this.authorsWithTexts.find((p) => p.texts.some((t) => t.authorId === id))
    ) {
      return;
    }

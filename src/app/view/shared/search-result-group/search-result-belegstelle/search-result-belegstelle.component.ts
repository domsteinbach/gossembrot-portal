...
export class SearchResultBelegstelleComponent implements OnChanges {
  ...
  ngOnChanges() {
    const belegstellenIds = this.results
      .filter((i) => i.type === "belegstelle")
      .map((r) => r.id);
    this._vr
      .getVerweiseOfSrcBelegstellen$(belegstellenIds)
      .pipe(take(1))
      .subscribe((verweise) => {
        this.belegstellen = this.results
          .map((result) => ({
            belegstelle: result,
    ...
    this.termsToHighlight = SearchService.expandWithAlternativeWritings(
      this.searchTerm,
    );
  }

...
import { SelectedPageState, UpdateSelectedPage } from '../../../state/app-state';
...
export class BlattSelectComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this._selectedPage$.pipe(takeUntil(this._destroy$)).subscribe((selectedPage: Page) => {
      if (selectedPage && selectedPage.pageText!== this.selectedPage?.pageText && this.pages.some(p => p.pageText === selectedPage.pageText)) {
  ...
  ngOnChanges(): void {
    if (this.pages.length > 0) {
      ...
      const page = this._store.selectSnapshot(SelectedPageState => SelectedPageState.selectedPage)

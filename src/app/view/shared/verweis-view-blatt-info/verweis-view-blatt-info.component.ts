...
export class VerweisViewBlattInfoComponent implements OnInit, OnDestroy {
  ...
  get blattangabe(): string {
    const belegstelle =
      this.agency === "Src"
        ? this.verweis?.srcBelegstelleObj
        : this.verweis?.targetBelegstelleObj;
    const physicality =
      this.agency === "Src"
        ? this.verweis?.srcCarObj?.physicality
        : this.verweis?.targetCarPhysicality;
    const prefix =
      !belegstelle?.lost && physicality === "Available" && this.agency === "Src"
        ? "Bl. "
        : "";
  ...
  get missingTooltip(): string {
    const alternativeBlattangabe = this.alternativePageBlattangabe
      ? ` Es wird stattdessen ${this.alternativePageBlattangabe} angezeigt.`
      : "";
    ...
    switch (this.agency) {
      case "Target":
        if (this.verweis?.targetCarObj?.physicality === "Available") {
          title = this.verweis?.targetCarObj?.shortName
            ? `${this.verweis?.targetCarObj?.shortName}, ${this.verweis.targetCarObj.shelfMark}`
            : this.verweis?.targetCarObj?.fullTitle;
        } else {
          title = this.verweis?.targetCarObj?.fullTitle || "";
        }
        break;
      case "Src":
        title = this.verweis?.srcCarObj?.shortName
          ? `${this.verweis?.srcCarObj?.shortName}, ${this.verweis?.srcCarObj?.shelfMark}`
          : this.verweis?.srcCarObj?.fullTitle;
        break;
    }
    return title || "";
  }
  ...
  get page(): Page | undefined {
    return this.agency === "Src"
      ? this.verweis?.srcBelegstelleObj?.page
      : this.verweis?.targetPage;
  }

  get isMissingPageOfExistingCarrier(): boolean {
    return (
      this.agency === "Target" &&
      this.verweis?.targetCarPhysicality === "Available" &&
      this.verweis?.targetBelegstelleObj?.lost === true
    );
  }

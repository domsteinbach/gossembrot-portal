...
export class IncomingVerweiseComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    if (this._route.snapshot.paramMap.get(RouteConstants.INFO_CARRIER_PARAM)) {
      this.carrierToSelect =
        this._route.snapshot.paramMap.get(RouteConstants.INFO_CARRIER_PARAM) ||
        "";
    }
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      const carrierToSelect = this.carriers.find(
        (c) => c.id === this.carrierToSelect,
      );
      if (carrierToSelect) {
  ...
  private _setCarrierParam(carId: string) {
    ...
    if (path[1] === RouteConstants.RECONSTRUCTION) {
      ...
      this._router.navigate([RouteConstants.RECONSTRUCTION, sub, carId], {

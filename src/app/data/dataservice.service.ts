...
export class DataService {
  ...
  getDataAs$<R, M>(
    Ctor: { new (r: R): M; tableName?: string },
    query?: string,
  ): Observable<M[]> {
    if (!Ctor.tableName && !query) {
      throw new Error(
        "The class passed must have a tableName property if no query is passed. Add the property in your class model or pass a query.",
      );
    }

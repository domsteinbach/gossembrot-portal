...
export class VerweisRepository {
  ...
  getVerweisePointingToCarrier$(car_id: string): Observable<DisplayVerweis[]> {
    ...
    const infoCars$: Observable<InformationCarrier[]> =
      ...
      ? this.verweise$().pipe(
          map((data) =>
            data.filter(
              (v) =>
                v.srcCar === car_id &&
                (includeErwaehnungen || v.type !== "Erwaehnung"),
            ),
          ),
        )
      : this._getOutgoingVerweiseFromCarrier$(car_id).pipe(
          map((data) =>
            data.filter((v) => includeErwaehnungen || v.type !== "Erwaehnung"),
          ),
        );

    const infoCarriers$: Observable<InformationCarrier[]> =
      this._store.select(CarriersState);

    return combineLatest([verweise$, infoCarriers$]).pipe(
      map(([verweise, carriers]) =>
        verweise.map((v: DisplayVerweis) => {
          v.srcCarObj = carriers.find(
            (c: InformationCarrier) => c.id === v.srcCar,
          );
          v.targetCarObj = carriers.find(
  ...
  getVerweiseByIds$(ids: string[] = []): Observable<DisplayVerweis[]> {
    ...
    if (this._cachedVerweise.length) {
      const cachedVerweise = this._cachedVerweise.filter((v) =>
        ids.includes(v.id),
      );
  ...
  getVerweiseOfSrcBelegstellen$(
    belegstellenIds: string[],
  ): Observable<DisplayVerweis[]> {
  ...
  private _getVerweisePointingToCarrierCached$(
    car_id: string,
  ): Observable<DisplayVerweis[]> {
    return this.verweise$().pipe(
      map((data) => {
  ...
  private _getOutgoingVerweiseFromCarrier$(
    car_id: string,
  ): Observable<DisplayVerweis[]> {
  ...
  private _getVerweisePointingToCarrier$(
    car_id: string,
  ): Observable<DisplayVerweis[]> {

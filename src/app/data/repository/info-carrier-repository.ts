...
export class InfoCarrierRepository {
  ...
  private _infoCarriers$ = this._dataService
    .getDataAs$(InformationCarrier, this._query)
    .pipe(filter((c) => c.length > 0));

  private _libraries$ = this._dataService
    .getDataAs$(Library)
    .pipe(filter((l) => l.length > 0));

  private _namingsOfGossembrot$ = this._dataService
    .getDataAs$(NamingGossembrot)
    .pipe(filter((c) => c.length > 0));
  ...
  getCarrierByNamingGsmbId$(
    namingId: string,
  ): Observable<InformationCarrier | undefined> {
    return this.informationCarriers$().pipe(
      switchMap((carriers) => {
        const foundCarrier = carriers.find((c) =>
          c.namingsGossembrot.some((n) => n.id === namingId),
        );
  ...
  private _getInformationCarriersWithNamings$(): Observable<
    InformationCarrier[]
  > {
      ...
      map(([carriers, namings, libraries, einbaende, externalEntities]) => {
        carriers.forEach((c: InformationCarrier) => {
          ...
          einbaende.forEach((e: Einband) => {
            if (e.carId === c.id) {
              e.externalEntity = externalEntities.find(
                (ee) => ee.objId === e.id && ee.objType === "Einband",
              );
            }
          });
          c.einbandInfo = einbaende.filter((e) => e.carId === c.id);
          c.externalDigitalisat = externalEntities.find(
            (ee) => ee.objId === c.id && ee.objType === "Digitalfaksimile",
          );
          c.handschriftenPortal = externalEntities.find(
            (ee) => ee.objId === c.id && ee.objType === "Handschriftenportal",
          );
          c.handschriftenCensus = externalEntities.find(
            (ee) => ee.objId === c.id && ee.objType === "Handschriftencensus",
          );
        });
        return carriers;
      }),

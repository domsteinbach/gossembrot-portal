...
import {
  ...
  SearchType,
} from "../data/repository/search-data-repository";
...
export class SearchService {
  ...
  private _restartSearch(searchTerm: string): void {
    ...
    const termsToSearch =
      this.searchType === "unset"
        ? SearchService.expandWithAlternativeWritings(this._startSearchTerm)
        : searchTerm;
    this._search
      .fullTextSearch$(termsToSearch, this.searchType)
      .pipe(take(1))
  ...
  private _filterAndApplySearchResults(searchTerm: string): void {
    ...
    const filteredResults = this._startSearchResults.filter((r) =>
      filterTerms.some((term) =>
        r.searchString.toLowerCase().includes(term.toLowerCase()),
      ),
    );
  ...
  static removeIgnoredCharacters(str: string): string {
    this.IGNORED_CHARACTERS.forEach((ignoredCharacter) => {
      const escapedCharacter = ignoredCharacter.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&",
      );

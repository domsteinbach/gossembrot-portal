...
export class LostPageNotificationService {
  constructor(
    ...
    const brightness: ThemeBrightness =
      this._store.selectSnapshot(SelectedThemeState).brightness;
    const panelClass: GsmbThemeClass =
      brightness === "dark" ? "dark-theme" : "old-brick-theme";
    this._store
      .select(SelectedVerweisState)
      .subscribe((verweis: DisplayVerweis) => {
        if (
          verweis &&
          verweis.targetBelegstelleObj &&
          verweis.targetCarPhysicality === "Available" &&
          verweis?.targetBelegstelleObj?.lost &&
